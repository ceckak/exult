...
void Shapes_vga_file::read_info(
	...
	auto shpdims = U7open2(patch_name(PATCH_SHPDIMS), SHPDIMS, editing);
	if (shpdims)
	...
	// WGTVOL
	auto wgtvol = U7open2(patch_name(PATCH_WGTVOL), WGTVOL, editing);
	if (wgtvol)
	...
	// TFA
	auto tfa = U7open2(patch_name(PATCH_TFA), TFA, editing);
	if (tfa)
	...
	auto wihh = U7open2(patch_name(PATCH_WIHH), WIHH, editing);
	if (wihh) {
	...
	auto occ = U7open2(patch_name(PATCH_OCCLUDE), OCCLUDE, editing);
	if (occ) {
	...
	// Get 'equip.dat'.
	auto mfile = U7open2(patch_name(PATCH_EQUIP), EQUIP, editing);
	if (mfile) {
